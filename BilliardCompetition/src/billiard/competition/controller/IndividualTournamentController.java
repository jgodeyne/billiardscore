 ...
 * @author jean
 */
public class IndividualTournamentController implements Initializable, ControllerInterface {
    ...
    @FXML
    private TextField tfCompetition;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cbTableFormat.getItems().addAll(new ArrayList(Arrays.asList(PermittedValues.TABLE_FORMAT)));
        cbTableFormat.getSelectionModel().select(1);

        //Participants Tab
        tabParticipants.setDisable(true);
        ...
        //Ranking Tab
        tabRanking.setDisable(true);
        tblColRankingLic.setCellValueFactory(new PropertyValueFactory<>("playerLic"));
        //tblColName.setCellValueFactory(new PropertyValueFactory<>();
    ...
    public void setTournament(IndividualTournament tournament) {
        if (tournament!=null) {
            ...
            tabParticipants.setDisable(false);
            tabMatches.setDisable(false);
            tabRanking.setDisable(false);
        }
    ...
    private void populateTournamentFields() {
        if (tournament!=null) {
            tfCompetition.setText(tournament.getName());
    ...
    public boolean validForm() {
        boolean valid=FormValidation.validateTextField(tfCompetition);
    ...
    private void save() throws Exception {
        if (validForm()) {
            if(tournament==null) {
                tournament = new IndividualTournament(tfCompetition.getText(), cbDiscipline.getValue(), cbTableFormat.getValue());
            } else {
                tournament.setName(tfCompetition.getText());
    ...
    private void matchUpdated(Match match) {
        ...
        if(matches.contains(match)) {
            ...
            if(match.isEnded()) {
                ...
                PointsSystemInterface pointSystem = PointSystemFactory.getPointSystem(PointSystemFactory.PointSystem.MATCHPOINTS_PROM);
